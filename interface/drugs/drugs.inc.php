...
// Return a warehouse id appropriate for the given user (or null).
// Priority: user's `default_warehouse` if valid, else first active warehouse for user's facility, else null.
function genWarehouse($userid = 0)
{
    if (!$userid) {
        $userid = $_SESSION['authUserID'];
    }

    $row = sqlQuery("SELECT default_warehouse, facility_id FROM users WHERE id = ?", array($userid));
    $default_wh = $row['default_warehouse'] ?? null;
    $fac_id = $row['facility_id'] ?? $GLOBALS['facility'];

    if ($default_wh) {
        $check = sqlQuery(
            "SELECT count(*) AS count FROM list_options WHERE list_id = 'warehouse' AND option_id = ? AND activity = 1",
            array($default_wh)
        );
    ...
    // Fallback: first active warehouse for user's facility (option_value stores facility id).
    $row2 = sqlQuery(
        "SELECT option_id FROM list_options WHERE list_id = 'warehouse' AND option_value = ? AND activity = 1 ORDER BY seq LIMIT 1",
        array($fac_id)
    );
    if ($row2 && !empty($row2['option_id'])) {
        return $row2['option_id'];
    }
