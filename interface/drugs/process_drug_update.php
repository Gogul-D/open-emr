<?php
/**
 * Processes drug update after ACK received
 */
...
// Function to process pending drug update transaction after ACK received
function processPendingDrugUpdateTransaction($msg_id) {
    ...
    if (!$pending) {
        error_log("No pending drug update transaction found for msg_id: " . $msg_id);
        return false;
    }
    
    $drug_data = json_decode($pending['transaction_data'], true);
    $drug_id = $pending['entity_id'];
    
    if (!$drug_data || !$drug_id) {
        error_log("Failed to decode drug update transaction_data for msg_id: " . $msg_id);
        return false;
    }
    ...
    try {
        // Now actually update the drug in database
        sqlStatement(
            "UPDATE drugs SET " .
            "name = ?, ndc_number = ?, drug_code = ?, on_order = ?, reorder_point = ?, max_level = ?, " .
            "form = ?, size = ?, unit = ?, route = ?, cyp_factor = ?, related_code = ?, " .
        ...
        error_log("Drug update ACK kept for audit: msg_id $msg_id");
        
        return $drug_id;
        
    } catch (Exception $e) {
        error_log("Database error in processPendingDrugUpdateTransaction: " . $e->getMessage());
        return false;
    }
}

// Process the drug update
try {
    $drug_id = processPendingDrugUpdateTransaction($msg_id);
    error_log("Drug update result for msg_id $msg_id: " . ($drug_id ? "Success - ID: $drug_id" : "Failed"));
} catch (Exception $e) {
    error_log("Error in processPendingDrugUpdateTransaction: " . $e->getMessage());
    $drug_id = false;
}
...
if ($drug_id) {
    ...
    echo "ERROR|Failed to update drug";
}
?>
