...
function getLBFForms($pid) {
    error_log("getLBFForms started for pid: $pid"); // Debug start
    ...
    // Debug log forms we're looking for
    error_log("Looking for forms: " . implode(", ", $relevantForms));
    ...
    $query = "SELECT f.id, f.encounter, f.form_name, f.form_id, f.formdir, COALESCE(fe.date, f.date) as date 
              ...
              WHERE f.pid = ? 
              AND f.formdir LIKE '%LBF%' 
              AND f.deleted = 0 
              ORDER BY COALESCE(fe.date, f.date) DESC";
              
    error_log("Executing query: $query with pid: $pid"); // Debug query
              
    $result = sqlStatement($query, array($pid));
    
    while ($row = sqlFetchArray($result)) {
        error_log("Processing form: {$row['form_name']} with form_id: {$row['form_id']}");
        
        if (in_array($row['form_name'], $relevantForms)) {
            error_log("Found relevant form: {$row['form_name']}");
            
            // Get all fields from lbf_data for this form
            $formFields = array();
            ...
            error_log("Looking for lbf_data with form_id: {$row['form_id']}"); // Debug lbf_data query
            
            // Create an associative array of field values
            while ($field = sqlFetchArray($fieldsQuery)) {
                error_log("Found field: {$field['field_id']} with value: {$field['field_value']}");
                if ($field['field_id'] != 'patient_sig') { // Skip signature field
                    $formFields[$field['field_id']] = $field['field_value'];
                }
            }
            
            error_log("Collected fields: " . print_r($formFields, true));
            ...
            //     error_log("Added diagnoses to content");
            // }
            
            // Only add form if we have content
            if (!empty($content)) {
                $header = $row['form_name'] . ' - (' . date('m/d/Y', strtotime($row['date'])) . ')';
                error_log("Creating form entry with header: $header");
                ...
                error_log("Added form to results array");
            }
        }
    }
    
    error_log("Returning " . count($forms) . " forms"); // Debug final count
    return $forms;
}
...
function getLBFNotesByFormName($pid, $formName) {
    $notes = [];
    // Get the latest form_id for the given patient and form name
    $sql = "SELECT f.form_id, f.date, f.encounter, f.user FROM forms f WHERE f.pid = ? AND f.form_name = ? AND f.deleted = 0 ORDER BY f.date DESC LIMIT 1";
    $row = sqlQuery($sql, [$pid, $formName]);
...
$response = [
    ...
    'vitals' => getLatestVitals($pid),
    'physical_exam' => getPhysicalExamFindings($pid),
    'labs' => getLatestLabs($pid, $labFilter),
    'allergies' => getAllergies($pid),
...
function getLatestVitals($pid) {
    $vitals = [];
    
    try {
        // Get vital signs from multiple dates - limit to last 10 encounters for performance
        $sql = "SELECT * 
                FROM form_vitals 
        ...
        if ($results === false) {
            error_log("Vitals query failed for PID: " . $pid);
            return [[
                'type' => 'vital',
...
function getPatientDetails($pid) {
    ...
    try {
        ...
        if ($result === false || !$result) {
            error_log("Patient details query failed for PID: " . $pid);
            return [[
                'type' => 'patient_detail',
...
/**
 * Get physical exam findings from form_physical_exam table
 */
...
function getLatestLabs($pid, $filter = 'lab_date') {
    ...
    try {
        // Get all lab orders with their results grouped by order
        $sql = "SELECT 
                    ...
                    procedure_result.result AS result,
                ...
                LEFT JOIN procedure_report ON procedure_report.procedure_order_id = procedure_order.procedure_order_id
                LEFT JOIN procedure_result ON procedure_result.procedure_report_id = procedure_report.procedure_report_id
                LEFT JOIN procedure_type ON procedure_result.result_code = procedure_type.procedure_code
                WHERE procedure_order.patient_id = ?
                ORDER BY procedure_order.date_ordered DESC, procedure_type.name ASC";
        
        $results = sqlStatement($sql, [$pid]);
        
        if ($results === false) {
            error_log("Lab results query failed for PID: " . $pid);
            return [[
                'type' => 'lab',
        ...
        while ($row = sqlFetchArray($results)) {
            ...
            $test_name = $row['name'] ?: $row['result_code'] ?: xl('Lab Result');
            
            if (!empty($row['result']) || !empty($row['result_text'])) {
                $result_value = trim($row['result'] ?: $row['result_text']);
                
                // Format individual result without including the order details
                $result_text = $test_name . ': ' . $result_value;
                if (!empty($row['units'])) {
                    $result_text .= ' ' . $row['units'];
                }
                if (!empty($row['abnormal'])) {
                    if (strtolower($row['abnormal']) === 'yes' || strtolower($row['abnormal']) === 'abnormal') {
                        $result_text .= ' [' . xl('Abnormal') . ']';
                    } elseif (strtolower($row['abnormal']) === 'no' || strtolower($row['abnormal']) === 'normal') {
                        $result_text .= ' [' . xl('Normal') . ']';
                    }
                }
            } else {
                // For pending labs that haven't been resulted yet
                $result_text = $test_name . ': ' . xl('Pending');
            }
            
            // Store just the result text without any order information
            $orders[$order_id]['results'][] = $result_text;
        }
        
        // Convert orders to labs array
        foreach ($orders as $order_id => $order) {
            // Create the header with all the order information
            $header_parts = [];
            $header_parts[] = xl('Lab Order');
            $header_parts[] = xl('Ordered') . ': ' . date('m/d/Y', strtotime($order['date_ordered']));
            if (!empty($order['date_collected'])) {
                $header_parts[] = xl('Collected') . ': ' . date('m/d/Y', strtotime($order['date_collected']));
            }
            if (!empty($order['order_diagnosis'])) {
                $header_parts[] = xl('Dx') . ' ' . $order['order_diagnosis'];
            }
            $header = implode(' | ', $header_parts);
            
            // Add ordered but pending notice if no results yet
            if (empty($order['results'])) {
                $order['results'][] = xl('Status') . ': ' . xl('Labs ordered, pending results');
            }

            // Format content with bullet points, containing only the lab results
            $content = "\n" . implode("\n", array_map(function($result) {
                ...
                'draggable_content' => $header . $content,  // Content already has the newline at the start
                'status' => empty($order['results']) ? 'pending' : 'completed'
            ];
        }
    } catch (Exception $e) {
        // If there are any database errors, log and continue
...
function getPatientHistory($pid) {
    ...
    try {
        ...
        if ($result === false) {
            error_log("Patient history query failed for PID: " . $pid);
            return [[
                'type' => 'history',
...
function getAllergies($pid) {
    ...
    try {
        ...
        if ($results === false) {
            error_log("Allergies query failed for PID: " . $pid);
            return [[
                'type' => 'allergy',
                'header' => xl('Allergies'),
        ...
        error_log("Allergy data query error: " . $e->getMessage());
        $allergies[] = [
            'type' => 'allergy',
            'header' => xl('Allergies'),
            'content' => 'NKDA (No Known Drug Allergies)'
...
function getPrescriptions($pid) {
    ...
    try {
        ...
        if ($results === false) {
            error_log("Prescriptions query failed for PID: " . $pid);
            return [[
                'type' => 'prescription',
        ...
        error_log("Prescription data query error: " . $e->getMessage());
        $prescriptions[] = [
            'type' => 'prescription',
            'header' => xl('Prescriptions'),
            'content' => xl('No current prescriptions documented')
...
function getMedications($pid) {
    ...
    try {
        ...
        if ($results === false) {
            error_log("Medications query failed for PID: " . $pid);
            return [[
                'type' => 'medication',
        ...
        error_log("Medication data query error: " . $e->getMessage());
        $medications[] = [
            'type' => 'medication',
            'header' => xl('Medications'),
            'content' => xl('No medications documented')
...
/**
 * Get medical problems using billing table data
 */
function getMedicalProblems($pid, $encounter = null) {
    $problems = [];
    
    try {
        // Get diagnoses from billing table matching pid and encounter
        $sql = "SELECT code_type, code, code_text, date 
                FROM billing 
                WHERE pid = ? AND encounter = ? 
                ORDER BY date DESC";
        $results = sqlStatement($sql, [$pid, $encounter]);
        
        if ($results === false) {
            error_log("Medical problems query failed for PID: " . $pid . ", Encounter: " . $encounter);
            return [[
                'type' => 'problem',
                'header' => xl('Diagnoses'),
                'content' => xl('No active diagnoses documented')
            ]];
        }
        
        while ($row = sqlFetchArray($results)) {
            // Get the diagnosis details
            $codeType = trim($row['code_type'] ?? '');
            $code = trim($row['code'] ?? '');
            $codeText = trim($row['code_text'] ?? '');
            $dateDiagnosed = (!empty($row['date']) && $row['date'] != '0000-00-00') ? date('m/d/Y', strtotime($row['date'])) : xl('Unknown');

            // Build content: Code Text (Code Type: Code)
            $content = $codeText;
            if (!empty($code)) {
                $content .= ' (' . $codeType . ': ' . $code . ')';
            }

            $problems[] = [
                'type' => 'problem',
                'header' => $dateDiagnosed,
                'content' => $content,
                'draggable_group' => 'diagnosis_' . md5($codeText . $code . $dateDiagnosed),
        ...
        error_log("Medical problems query error: " . $e->getMessage());
        $problems[] = [
            'type' => 'problem',
            'header' => xl('Diagnoses'),
            'content' => xl('No active diagnoses documented')
...
/**
 * Get Review of Systems data from form_ros table
 */
function getReviewOfSystems($pid) {
    ...
    try {
        ...
        $ros_data = sqlQuery($sql, [$pid]);
        
        if ($ros_data === false) {
            error_log("ROS query failed for PID: " . $pid);
            return [[
                'type' => 'ros',
            ...
            foreach ($ros_systems as $system_name => $fields) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (strtoupper($value) === 'YES') {
                        $positive_findings[] = $display_name;
                    } elseif (strtoupper($value) === 'NO') {
                        $negative_findings[] = $display_name;
                    }
                }
                ...
                if (!empty($positive_findings) || !empty($negative_findings)) {
                    ...
                    if (!empty($positive_findings)) {
                        $content_parts[] = 'Positive: ' . implode(', ', $positive_findings);
                    }
                    
                    if (!empty($negative_findings)) {
                        $content_parts[] = 'Negative: ' . implode(', ', $negative_findings);
                    }
                    
                    $rosData[] = [
                        'type' => 'ros',
                        'header' => $system_name . ' ROS',
