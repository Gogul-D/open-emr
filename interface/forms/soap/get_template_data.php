...
try {
    ...
    switch ($action) {
        ...
        case 'get_templates':
            ...
            if ($context && $category) {
                $sql = "SELECT id, template_name, context, category, created_by FROM soap_custom_templates WHERE context = ? AND category = ? AND is_active = 1 ORDER BY template_name";
                $result = sqlStatement($sql, array($context, $category));
            } elseif ($context) {
                $sql = "SELECT id, template_name, context, category, created_by FROM soap_custom_templates WHERE context = ? AND is_active = 1 ORDER BY template_name";
                $result = sqlStatement($sql, array($context));
            } else {
                $sql = "SELECT id, template_name, context, category, created_by FROM soap_custom_templates WHERE is_active = 1 ORDER BY template_name";
                $result = sqlStatement($sql);
            }
            ...
            error_log("[Template Debug] Starting template fetch for user: $currentUser");
            
            while ($row = sqlFetchArray($result)) {
                error_log("[Template Debug] Processing template row: " . json_encode($row));
                
                // Ensure we have the expected columns
                $templateId = $row['id'] ?? null;
                $templateName = $row['template_name'] ?? 'Unknown Template';
                $templateContext = $row['context'] ?? 'Unknown Context';
                $templateCategory = $row['category'] ?? 'Unknown Category';
                $createdBy = $row['created_by'] ?? null;
                
                error_log("[Template Debug] Extracted values - ID: $templateId, Name: $templateName, Context: $templateContext, Category: $templateCategory");
                ...
                if ($currentUser) {
                    ...
                    try {
                        ...
                        if ($roleQuery && !empty($roleQuery['role'])) {
                            ...
                            $isAdmin = in_array('admin', $userRoles) || in_array('super', $userRoles) || 
                                       in_array('administrator', $userRoles) || in_array('administrators', $userRoles);
                    ...
                    $canDelete = $isAdmin || ($createdBy == $currentUser);
                }
                ...
                error_log("[Template Debug] Adding template: " . json_encode($templateData));
                $templates[] = $templateData;
            }
            
            error_log("[Template Debug] Total templates found: " . count($templates));
