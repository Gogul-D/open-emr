...
function getLatestLabs($pid, $filter = 'lab_date') {
    ...
    try {
        ...
        $sql = "SELECT
                    ...
                    procedure_type.name,
                    procedure_order_code.procedure_code AS order_code,
                ...
                LEFT JOIN procedure_order_code ON procedure_order_code.procedure_order_id = procedure_order.procedure_order_id
                WHERE procedure_order.patient_id = ?
                ORDER BY procedure_order.date_ordered DESC, procedure_type.name ASC, procedure_order_code.procedure_name ASC";
        ...
        while ($row = sqlFetchArray($results)) {
            ...
            $test_name = $row['name'] ?: $row['procedure_name'] ?: $row['result_code'] ?: $row['order_code'] ?: xl('Lab Result');
            
            // Collect test names - prioritize procedure_name from order, then name from result
            if (!empty($row['procedure_name']) && !in_array($row['procedure_name'], $orders[$order_id]['test_names'])) {
                $orders[$order_id]['test_names'][] = $row['procedure_name'];
            }
            if (!empty($row['name']) && !in_array($row['name'], $orders[$order_id]['test_names'])) {
                $orders[$order_id]['test_names'][] = $row['name'];
            }
            
            if (!empty($row['result']) || !empty($row['result_text'])) {
                $result_value = trim($row['result'] ?: $row['result_text']);
                
                // Format individual result without including the order details and without units
                $result_text = $test_name . ': ' . $result_value;
                // Removed units from display as requested
            ...
            // Note: Pending results are handled later in the foreach loop
        }
        
        // Convert orders to labs array
        foreach ($orders as $order_id => $order) {
            // Always try to get test names from procedure_order_code first
            $testSql = "SELECT procedure_name FROM procedure_order_code WHERE procedure_order_id = ? AND procedure_name IS NOT NULL AND procedure_name != ''";
            $testResults = sqlStatement($testSql, [$order_id]);
            while ($testRow = sqlFetchArray($testResults)) {
                if (!in_array($testRow['procedure_name'], $order['test_names'])) {
                    $order['test_names'][] = $testRow['procedure_name'];
                }
            }
            
            // Ensure we have test names - if not, add a generic one
            if (empty($order['test_names'])) {
                $order['test_names'][] = xl('Lab Test');
            }
            
            // Add ordered but pending notice if no results yet
            if (empty($order['results'])) {
                // Show generic "Lab Result: Pending" for pending orders
                $order['results'][] = xl('Lab Result') . ': ' . xl('Pending');
            }
            
            // Create the header with all the order information
            $header_parts = [];
            
            // Use test names for header when available, otherwise use "Lab Order"
            if (!empty($order['test_names'])) {
                $header_parts[] = implode(', ', $order['test_names']);
            } else {
...
function getMedicalProblems($pid, $encounter = null) {
    ...
    try {
        ...
        while ($row = sqlFetchArray($results)) {
            ...
            if ($billingResult) {
                while ($billingRow = sqlFetchArray($billingResult)) {
                    if (!empty($billingRow['code'])) {
                        $icd10Codes[] = trim($billingRow['code']);
                    }
                }
            }

            // Use the most recent ICD-10 codes found
            $icd10Code = '';
            $icd10Text = '';
            if (!empty($icd10Codes)) {
                // Prefix each code with "ICD10:" before joining
                $prefixedCodes = array_map(function($code) {
                    return 'ICD10:' . $code;
                }, $icd10Codes);
                $icd10Code = implode(' ; ', $prefixedCodes);
                // For display, we can show the codes without individual descriptions
                // since we're showing multiple codes
            }

            // Build content: Problem title with ICD-10 code if available
            $content = $title;
            if (!empty($diagnosis) && $diagnosis != $title) {
                $content .= ' - ' . $diagnosis;
            }

            // Add ICD-10 codes if found (from fee sheet)
            if (!empty($icd10Code)) {
                $content .= ' - ' . $icd10Code;
            }
            ...
            $problems[] = [
                'type' => 'problem',
                'header' => !empty($dateInfo) ? $dateInfo : xl('Medical Problem'),
                'content' => $content,
