...
function saveNotes(pid, eid) {
    var notes = $("#msg_notes_" + pid).val();
    var $button = $("button.btn-save-note[data-pid='" + pid + "']");
    
    console.log('saveNotes called with pid:', pid, 'eid:', eid);
    ...
    // Restore session to avoid timeout issues
    top.restoreSession();
    
    var ajaxData = {
        'parameter': JSON.stringify([pid]),
        'pc_eid': JSON.stringify([eid]),
        'uid_pc_eid': JSON.stringify([pid + '_' + eid]),
        'msg_notes': notes,
        'action': 'process',
        'item': 'notes'
    };
    
    console.log('Sending AJAX POST to save.php with data:', ajaxData);
    ...
    $.ajax({
        ...
        success: function(result) {
            console.log('AJAX success! Response received:', result);
            console.log('Response type:', typeof result);
            // Show success feedback
            $button.text('Note Saved!').addClass('btn-success').removeClass('btn-primary');
            
            // Immediately display the saved note in the Progress column
            if (notes.trim() !== '') {
                var $row = $button.closest('tr');
                
                // Find the Progress column in this row
                var $progressCol = $row.find('td.msg_progress');
                
                console.log('Progress column found:', $progressCol.length);
                
                if ($progressCol.length > 0) {
                    // Create note HTML matching the format from show_progress_recall()
                    var now = new Date();
                    var dateStr = (now.getMonth() + 1) + '/' + now.getDate() + '/' + now.getFullYear() + 
                                  ' @ ' + now.getHours() + ':' + (now.getMinutes() < 10 ? '0' : '') + now.getMinutes();
                    
                    var noteHtml = "<span class='left' data-toggle='tooltip' data-placement='auto' title='Note saved at " + dateStr + "'>" +
                                   "<b>Note:</b> " + notes + "</span><br />";
                    
                    // Always append to the end of the column (below all existing notes and content)
                    $progressCol.append(noteHtml);
                    console.log('Note displayed in Progress column');
                }
            }
            
            // Revert button after 2 seconds
            setTimeout(function() {
                $button.prop('disabled', false).text(originalText).removeClass('btn-success').addClass('btn-primary');
            }, 2000);
        },
        error: function(jqXHR, textStatus, errorThrown) {
            // Show error feedback
            console.log('AJAX error:', textStatus, errorThrown, 'Response:', jqXHR.responseText);
            $button.text('Error Saving').addClass('btn-danger').removeClass('btn-primary');
...
// Event delegation for Save Note buttons
$(document).on('click', 'button.btn-save-note', function(e) {
    e.preventDefault();
    var pid = $(this).data('pid');
    var eid = $(this).data('eid');
    console.log('Button clicked! pid:', pid, 'eid:', eid);
    saveNotes(pid, eid);
});
...
function show_this(colorish='') {
    ...
    $('.ALL').each(function() { this.style.display = 'none'; });
    $('.ALL').filter(function () {
        var d = $(this).data();
    ...
    }).each(function() { this.style.display = 'table-row'; });
}
