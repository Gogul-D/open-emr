<?php /* Smarty version 2.6.33, created on 2025-03-24 17:31:19
         compiled from default/views/week_print/outlook_ajax_template.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/week_print/outlook_ajax_template.html', 10, false),array('function', 'headerTemplate', 'default/views/week_print/outlook_ajax_template.html', 198, false),array('function', 'pc_sort_events', 'default/views/week_print/outlook_ajax_template.html', 250, false),array('modifier', 'date_format', 'default/views/week_print/outlook_ajax_template.html', 245, false),array('modifier', 'string_format', 'default/views/week_print/outlook_ajax_template.html', 246, false),)), $this); ?>
...
foreach ($providers as $provider) {
    ...
    echo xlt(date('F', strtotime($atmp[0]))) . " "  . text(date('d', strtotime($atmp[0])));
    echo " - <br />";
    echo xlt(date('F', strtotime($atmp[count($atmp)-1]))) . " "  . text(date('d', strtotime($atmp[count($atmp)-1])));
    echo "</span>";
    echo "</div>";
    ...
    foreach ($A_EVENTS as $date => $events) {
        ...
        echo "<td><div id='dowheader'>". xlt(date("D",strtotime($eventdate))) . " " . text(date("m/d", strtotime($eventdate))) . "</div>";

        // output the events for this date
        PrintEvents($events, $providerid, ($TPL_IMAGE_PATH ?? null));
        ...
        if (isset($A_EVENTS[date("Y-m-d", $dateplusfour)])) {
            echo "<div id='dowheader'>". xlt(date("D",$dateplusfour)) . " " . text(date("m/d", $dateplusfour)) . "</div>";
            $tmpEvents = $A_EVENTS[date("Y-m-d", $dateplusfour)];
            PrintEvents($tmpEvents, $providerid, ($TPL_IMAGE_PATH ?? null));
        }
        echo "</tr>";
...
function PrintEvents($events, $providerid, $TPL_IMAGE_PATH) {
    global $times, $calStartMin, $calEndMin, $timeslotHeightVal, $timeslotHeightUnit;
	    ...
	    $fname = substr($event['patient_name'], $commapos + 2);
            ...
            $divTitle = date("D, d M Y", strtotime($date ?? null));
            ...
            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11) {
                ...
                $content .= text(date("h:i", $eventdatetime));
            ...
            echo "<div data-eid='" . attr($eventid) . "' class='" . attr($evtClass) . " event' style='background-color:".$event["catcolor"].
                    "' ".
