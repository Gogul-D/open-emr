...
function verifyEmail(string $languageChoice, string $fname, string $mname, string $lname, string $dob, string $email): bool
{
    ...
    // print_r($languageChoice." ".$fname." ".$mname." ".$lname." ".$dob." ".$email); die();
    ...
    if (!empty($sql['pid'])) {
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            if (empty($token_encrypt)) {
                ...
                print_r("token encryption failed"); die();
        ...
        if (!$unique) {
            print_r("was unable to create a unique token in verifyEmail function, so failed"); die();
    ...
    if ($emailPrepSend) {
        ...
        if ($mail->Send()) {
            //   print_r("hit below mail success"); die();
            ...
            //   print_r("hit below mail failure"); die();
