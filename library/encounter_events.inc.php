...
function todaysEncounterCheck($patient_id, $enc_date = '', $reason = '', $fac_id = '', $billing_fac = '', $provider = '', $cat = '', $return_existing = true)
{
    ...
    addForm(
        ...
        sqlInsert(
            ...
            "pc_catid = ?," .
	    "pos_code = ?," .
	    "class_code = (SELECT option_id FROM list_options WHERE list_id = ? AND title" .
            "= (SELECT pc_catname FROM openemr_postcalendar_categories WHERE openemr_postcalendar_categories.pc_catid = ? LIMIT 1) LIMIT 1)," .
	    "encounter_type_code = (SELECT option_id FROM list_options WHERE list_id = ? AND title" . 
	    "= (SELECT pc_catname FROM openemr_postcalendar_categories WHERE openemr_postcalendar_categories.pc_catid = ? LIMIT 1) LIMIT 1)," .
	    "encounter_type_description = (SELECT pc_catname FROM openemr_postcalendar_categories WHERE openemr_postcalendar_categories.pc_catid = ? LIMIT 1);",
            array($dos,$visit_reason,$facility,$facility_id,$billing_facility,$visit_provider,$patient_id,$encounter,$visit_cat, $pos_code, '_ActEncounterCode', $visit_cat, 'encounter-types', $visit_cat, $visit_cat)
        ),
        "newpatient",
        $patient_id,
        "1",
