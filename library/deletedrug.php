...
if ((!empty($id)) && ($id > 0)) {
    ...
    // Generate HL7 cancel message before deleting
    if (!empty($dn['patient_id'])) {
        require_once "../controllers/C_Prescription.class.php";
        require_once "classes/Prescription.class.php";
        
        // Load the actual prescription object
        $prescription = new Prescription($id);
        
        // Remove medication list entries for component drugs if this is a packet prescription
        $prescription->remove_packet_component_medications();
        
        // Generate HL7 cancel message
        $prescriptionController = new C_Prescription();
        $hl7_message = $prescriptionController->generatePrescriptionDeleteHL7($prescription);
        $prescriptionController->savePrescriptionHL7ToOutbound($hl7_message, $id, 'delete');
    }
